...
The key design feature of the tensor is automatic garbage collection (GC). In
C++, the class `at::Tensor` contains only one data member, a `shared_ptr` to the
implementation of the tensor type.  The smart pointer `shared_ptr` implements
reference count-based GC, which frees a tensor once its reference count becomes
zero.  Comparing with Go and Java’s GC, which runs the mark-and-sweep algorithm,
...
immediately without waiting for the completion of GC.  If all tensors are in CPU
memory, this mechanism works; however, in deep learning, we would prefer to host
tensors in GPU memory, which is a precious resource. We prefer free tensors
immediately when they are out-of-use so that the next iteration can create new
tensors in GPU.
...
We started with inventing new GC mechanisms in the library, including adding a
global reference count table. However, after trying several strategies, we
noticed that we could tailor Go’s GC for the tensor type in GoTorch specifically
to make it synchronous, or able to wait till the completion of GC before
returning.
...
The tailored Go GC mechanism doesn’t handle the first two categories, which is
the topic of the next section, Porting Modules.

To handle intermediate results, GoTorch users need to call `gotorch.GC()` at the
beginning of each train loop step.  The first job of `gotorch.GC()` is to mark
that all tensors generated since then, which are considered intermediate
results, are subject to the tailored GC.  After the train loop, users are
supposed to call `gotorch.FinishGC()` to unset the mark.
...
The second job of `gotorch.GC` is to call `runtime.GC()` and return after the
waiting group is unset. The call to `runtime.GC()` trigger’s Go’s mark-and-sweep
algorithm.  The waiting stops after the algorithm frees all tenors created since
the marking operation. Usually, the waiting takes less than one millisecond
(ms).
...
We see that `torch::randn` defined in libtorch returns an `at::Tensor`. However,
the Cgo wrapper `RandN` returns a C string representing the possible C++
exception.  The last parameter of `RanN` is the returned tensor.
...
Users can define PyTorch modules in Python and C++, but mostly in Python.

To define a new module in Python, users derive a class from `torch.nn.Module`,
adding data members corresponding to parameters, buffers, and sub-modules, and
attaching the method `forward` that defines the forward pass of the module.

The Python module-definition framework have Python decorators that call C++ core
functions `Module::register_{parameter|buffer|module}`.  This registration
mechanism is necessary as later users might call the module class’ methods like
`To` and `ZeroGrad`, which must know what tensors need to be moved to the
specified device, or which tensors are parameters and whose gradient tensors
need to be cleared.

Similarly, GoTorch allows users to define modules in Go. Users define a struct
type, say `LinearModule` that embeds the struct `gotorch/nn.Module`, which has
useful utility methods attached.

We don’t want to bother users to call the `register_xxx` function to specify the
role of eac data member.  So, in the definition of type `LinearModule`, we allow
users to specify Go struct field tags to tell the role.  If a field of type
`gotorch.Tensor` is tagged with `gotorch:buffer`, it is a buffer; otherwise, it
is a parameter.  Any field of type `gotorch/nn.Module` is considered a
sub-module.

Each module might have its Forward method in different signatures.  For example,
most Forward methods take one tensor-typed parameter and return a
tensor. However, some other modules can have various numbers of parameters and
return values in various types. GoTorch doesn’t require the Forward method in
any specified signature; instead, please feel free to define whatever form of
Forward method as you need -- the only requirement is that the method name is
Forward, so that the framework know how to call it from module containers like
`nn.Sequential`.

A typical example of GoTorch module definition is in `gotorch/nn.BatchNorm` in `
nn/batchnorm.go`.
...
Please be aware that RunningMean and RuningVar are buffers, and Weight and Bias
are parameters.  Also, please be aware that each module type must embed
`nn.Module` -- this is the Go way to class hierarchy.

Following the naming convention of PyTorch, each GoTorch module’s newer function
is not named `NewModuleName`; instead, we use `ModuleName`.  The following is
the newer function of the BatchNorm module.
...
Please be aware that the newer must call the method
`gotorch/nn.Module.Init`. This actually stores the newly created module’s
address into the field `outer` of the struct `gotorch/nn.Module`. Later, when
users calls `BatchNorm2dModule`’s `To` or `ZeroGrad` methods, they actually
calls methods with the name but attached to `gotorch/nn.Module`, which could
then know the parameters, buffers, and sub-modules defined in
`BatchNorm2dModule` by following the pointer `outer`.
